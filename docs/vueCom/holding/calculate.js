...
export function calFare({o, co, fareMap, marketValue, exchangeRateHKDtoUSD, HKStockExchangeRateList, accountTypeMap}) {
    let fare = '0', fareText = '';

    if(!!o.wtAccountType && !!fareMap?.[o.wtAccountType]?.[o.stockCodeType]){
        if(o.chiCang > 0){
            var totalFare = {};
            ...
            if(o.wtAccountType === 'R' || o.stockCodeType === 'h' || accountTypeMap['0_HK'].includes(o.wtAccountType)){
                ...
                fare1EX += `
                    因为是属于${textType}，印花税向上取整 Math.ceil(fare1) ，<span class='fontWeight'>故最终计算结果为${fare1}</span><br />
                `
            }
